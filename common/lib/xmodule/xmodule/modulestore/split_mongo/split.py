...
from xmodule.util.misc import get_coursekey_library
...
class SplitBulkWriteMixin(BulkOperationsMixin):
    ...
    def _get_bulk_ops_record(self, course_key, ignore_case=False):
        ...
        if course_key.org is None or get_coursekey_library(course_key) is None or course_key.run is None:
    ...
    def _clear_bulk_ops_record(self, course_key):
        ...
        if course_key.org and get_coursekey_library(course_key) and course_key.run:
...
class SplitMongoModuleStore(SplitBulkWriteMixin, ModuleStoreWriteBase):
    ...
    def _lookup_course(self, course_key, head_validation=True):
        ...
        if head_validation and course_key.org and get_coursekey_library(course_key) and course_key.run:
            if course_key.branch is None:
                raise InsufficientSpecificationError(course_key)
    ...
    def _create_courselike(  # lint-amnesty, pylint: disable=too-many-statements
        ...
        with self.bulk_operations(locator):
            ...
            index_entry = {
                '_id': ObjectId(),
                'org': locator.org,
                'course': get_coursekey_library(locator),
                'run': locator.run,
                'edited_by': user_id,
                'edited_on': datetime.datetime.now(UTC),
    ...
    def _get_index_if_valid(self, course_key, force=False):
        ...
        by course_key
        """
        if course_key.org is None or get_coursekey_library(course_key) is None or course_key.run is None or course_key.branch is None:
            return None
        else:
